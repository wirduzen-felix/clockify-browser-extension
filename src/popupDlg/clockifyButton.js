...
var clockifyButton = {
    ...
    createButton: (description, project, task, billable, canClose) => {
        const options = objectFromParams(description, project, task, billable);
    ...
    createInput: (options) => {
        ...
        input.setAttribute("placeholder", "Zeit hinzufügen (z.B. 15m)");
                ...
                if (m) {
                    ...
                    aBrowser.runtime.sendMessage({
                        ...
                        if (!response) {
                            ...
                            inputMessage(input, "Zeit hinzugefügt!", "success");
                        }
                    });
                } else {
...
function objectFromParams(description, project, task, billable) {
    ...
    try {
        if (title && title === clockifyButton.inProgressDescription) {
            ...
            aBrowser.runtime.sendMessage({
                ...
                if (!response) {
                    _waitingForResponse = false;
                    alert("Du musst angemeldet sein um Zeiten zu erfassen. (startWithDescription)");
                    // this.hideClockifyButtonLinks();
                    return;
                }
                else if (typeof response === 'string') {
